...
# Versione aggiornata al 13-03: risolto bug moltiplicazione capi con CI>1 | riga 70
...
def importa_BOM(df):
    ...
    for i in range(len(df)):
        ...
        if coefficiente != 1:
            ...
            df['CI'].iloc[i] = 1
            df['Splittato'].iloc[i]='*'
            for k in range(1,coefficiente):
                l = len(df) # aggiunto il 13-03 per bug moltiplicazione capo
                df.loc[l]=df.iloc[i]
...
if 'Finito' not in list(st.session_state.dizionario.keys()): # mi propone la form solo se nei componenti non c'è Finito. è il modo per terminare il grafo
    with st.form(key='my_form',clear_on_submit=True): 
        ...
        componenti = st.multiselect('**Selectați componente**', list(st.session_state.dizionario.keys()), key = 'my_componenti')
        descrizione_operazione = st.text_input('Descrierea operațiunii', key = 'my_descrizione')
        macchina = st.selectbox('Alegeți mașina',lista_macchine, index=None, key = 'my_macchina')
        tempo_ciclo = st.number_input('Introduceți durata ciclului:',min_value=0.00, key = 'my_tempo_ciclo')
        semilavorato = st.text_input('Descriere produs semifinit (Finito să termine ciclul)', key = 'my_semilavorato')
